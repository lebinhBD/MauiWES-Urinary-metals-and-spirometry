...
data <- data %>% 
  left_join(df,
            by = "participant_id")
sum(is.na(data$duration_months))
...
results2i_l90 <- gwqs(fvcbelowlln_N ~ pwqs + nwqs + age + male +  tobaco + BMIscore + duration_months,
                      mix_name = metals,
                      data = data_fvc, 
                      q = 4, 
                      validation = 0.6, 
...
df_fvc_2iwqs <- df_fvc_2iwqs[-c(1,4:8),]
...
fev2i_l90 <- gwqs(fev1belowlln_N ~ pwqs + nwqs + age + male +  tobaco + BMIscore+ duration_months,
                  mix_name = metals,
                  data = data_fev1, 
                  q = 4, 
                  validation = 0.6, 
...
df_fev_2iwqs <- df_fev_2iwqs[-c(1,4:8),]
...
ff2i_l90 <- gwqs(ffbelowlln_N ~ pwqs + nwqs + age + male +  tobaco + BMIscore + duration_months,
                 mix_name = metals,
                 data = data_ff, 
                 q = 4, 
                 validation = 0.6, 
...
df_ff_2iwqs <- df_ff_2iwqs[-c(1,4:8),]
...
fef2i_l90 <- gwqs(fef2575belowlln_N ~ pwqs + nwqs + age + male +  tobaco + BMIscore + duration_months,
                  mix_name = metals,
                  data = data_fef, 
                  q = 4, 
                  validation = 0.6, 
...
df_fef_2iwqs <- df_fef_2iwqs[-c(1,4:8),]
df_fef_2iwqs$term
...
#### combine images ####
library(magick)
img1 <- image_read("2iwqs_fvc_duration.png")
img2 <- image_read("2iwqs_fev_duration.png")
img3 <- image_read("2iwqs_ff_duration.png")
img4 <- image_read("2iwqs_fef_duration.png")
